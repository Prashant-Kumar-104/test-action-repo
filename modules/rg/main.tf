...
locals {
  name_1     = "ussite01"
  name_2     = "ussite02"
  location_1 = "eastus" # Specific location for Management 1
  location_2 = "westus" # Specific location for Management 2
}
...
resource "azurerm_resource_group" "resourcegroup_1" {
  name     = "${local.name_1}-${local.location_1}-rg-1"
  location = local.location_1
}

resource "azurerm_virtual_network" "virtualnetwork_1" {
  depends_on          = [azurerm_resource_group.resourcegroup_1]
  name                = "${local.name_1}-${local.location_1}-vnet-1"
...
resource "azurerm_subnet" "subnet_1" {
  depends_on           = [azurerm_virtual_network.virtualnetwork_1]
  name                 = "${local.name_1}-${local.location_1}-subnet-1"
...
resource "azurerm_resource_group" "resourcegroup_2" {
  name     = "${local.name_2}-${local.location_2}-rg-2"
  location = local.location_2
}

resource "azurerm_virtual_network" "virtualnetwork_2" {
  depends_on          = [azurerm_resource_group.resourcegroup_2]
  name                = "${local.name_2}-${local.location_2}-vnet-2"
...
resource "azurerm_subnet" "subnet_2" {
  depends_on           = [azurerm_virtual_network.virtualnetwork_2]
  name                 = "${local.name_2}-${local.location_2}-subnet-2"
...
resource "azurerm_log_analytics_workspace" "law_1" {
  name                = "${local.name_1}-${local.location_1}-logs-1"
  location            = azurerm_resource_group.resourcegroup_1.location
  resource_group_name = azurerm_resource_group.resourcegroup_1.name
  sku                 = "PerGB2018"
...
resource "azurerm_key_vault" "keyvault_1" {
  name                        = "${local.name_1}-${local.location_1}-kv-1"
  location                    = azurerm_resource_group.resourcegroup_1.location
  resource_group_name         = azurerm_resource_group.resourcegroup_1.name
...
resource "azurerm_network_interface" "virtualmachine_1" {
  depends_on          = [azurerm_virtual_network.virtualnetwork_1, azurerm_subnet.subnet_1]
...
resource "azurerm_network_interface" "virtualmachine_2" {
  depends_on          = [azurerm_virtual_network.virtualnetwork_2, azurerm_subnet.subnet_2]
